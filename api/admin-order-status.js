...
export default async function handler(req,res){
 ...
 try{
  ...
  if(pe)return json(res,500,{error:'Não foi possível verificar a permissão administrativa.'});
  ...
  if(!id||!valid.includes(status))return json(res,400,{error:'Pedido ou status inválido.'});
