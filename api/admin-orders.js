...
export default async function handler(req,res){
  ...
  try{
    ...
    if(ae){console.error('admin-orders permission:',ae);return res.status(500).json({error:'Não foi possível verificar a permissão administrativa.'});}
    if(!allowed)return res.status(403).json({error:'Acesso negado.'});
    const {data,error}=await db.from('orders').select('id,user_id,robux,method,amount,status,roblox_username,gamepass_url,created_at,updated_at,delivered_at,delivered_by').order('created_at',{ascending:false}).limit(200);
  ...
  }catch(e){console.error('admin-orders:',e);return res.status(500).json({error:e?.message||'Erro interno.'});}
}
