...
const env=n=>{if(!process.env[n])throw Error(`Missing ${n}`)};
export default async function handler(req,res){
 try{
  ...
  if(!role||!['LIDER','ADMINISTRADOR'].includes(role.role))return res.status(403).json({error:'Seu cargo não possui permissão para esta ação.'});
  if(req.method==='GET'){
   const {data,error}=await db.from('profiles').select('id,display_name,username,roblox_username,avatar_url,account_status,suspended_until,moderation_reason,created_at').order('created_at',{ascending:false}).limit(1000);
   if(error)return res.status(500).json({error:'Não foi possível carregar os usuários.'});return res.status(200).json({users:Array.isArray(data)?data:[]});
  }
  ...
  if(ue)return res.status(500).json({error:ue.message});if(!updated)return res.status(404).json({error:'Usuário não encontrado.'});
  ...
  if(ae2)return res.status(500).json({error:'Punição aplicada, mas não foi possível registrar a ação: '+ae2.message});
  ...
  if(le)return res.status(500).json({error:'Punição aplicada, mas não foi possível registrar o log: '+le.message});
