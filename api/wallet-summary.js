...
export default async function handler(req,res){if(req.method!=='GET')return json(res,405,{error:'Método não permitido.'});try{
 const token=String(req.headers.authorization||'').replace(/^Bearer\s+/i,'').trim();const user=await getUser(token);const userDb=client(process.env.SUPABASE_ANON_KEY,token);const adminDb=client(process.env.SUPABASE_SERVICE_ROLE_KEY);
 let summary=null;const rpc=await userDb.rpc('get_wallet_summary');if(!rpc.error&&rpc.data)summary=Array.isArray(rpc.data)?rpc.data[0]||{}:rpc.data;
 const [acc,deps,withs,tx,med]=await Promise.all([
  ...
  adminDb.from('med_cases').select('id,user_id,amount,status,reason,justification,review_reason,created_at,updated_at').eq('user_id',user.id).order('created_at',{ascending:false}).limit(100)
 ...
 if(!summary){const balance=Number(acc.data?.balance||0),pendingMed=sum(medCases.filter(x=>['AWAITING_JUSTIFICATION','UNDER_REVIEW'].includes(String(x.status).toUpperCase())),'amount'),pendingWithdrawals=sum(withdrawals.filter(x=>['PENDING','PROCESSING'].includes(String(x.status).toUpperCase())),'amount');summary={balance,available:Math.max(balance-pendingMed,0),pending_med:pendingMed,pending_withdrawals:pendingWithdrawals,total_deposits:sum(deposits.filter(x=>String(x.status).toUpperCase()==='PAID'),'amount'),total_withdrawals:sum(withdrawals.filter(x=>String(x.status).toUpperCase()==='PAID'),'amount')};}
